...
bool extract_operands(line_info line, int index, int* operand_count, char operands[2][MAX_LINE_LENGTH])
{
    ...
    while (line.content[index] != '\n' && line.content[index] != EOF) {
        ...
        for (i = 0;
            line.content[index] && !isspace(line.content[index]) && line.content[index] != ','; index++, i++) {
...
addressing_type get_addressing_type(char* operand)
{
    ...
    if (operand[0] == '#' && is_valid_data_parameter(operand + 1))
        return IMMEDIATE_ADDR;

    if (operand[0] == '*' && get_register_by_name(operand + 1) != NONE_REG)
        return INDIRECT_REGISTER_ADDR;

    if (get_register_by_name(operand) != NONE_REG)
        return DIRECT_REGISTER_ADDR;
